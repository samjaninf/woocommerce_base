...
class wpie_coupon {
    ...
    function __construct() {
        add_action('wp_ajax_wpie_get_filter_coupon_results', array(&$this, 'wpie_get_filter_coupon_results'));
    ...
    function wpie_get_filter_coupon_results() {
        global $wpie_product, $wpie_product_cat, $wpie_coupon;

        $wpie_product->wpie_set_time_limit(0);

        $coupon_list = $wpie_coupon->get_filter_coupon($_POST);
        ...
        <div class="wpie_filter_data_container">
            ...
            <table class="wpie_coupon_filter_data">
                <thead>
                    <tr>
                        <th></th>
                        ...
                        foreach ($coupon_list as $coupon_data) {
                            ...
                            <tr>
                                ...
                                foreach ($coupon_fields['coupon_fields'] as $coupon_info) {
                                    $temp_data = $coupon_info['field_key'];
                                    ?>
                                    <td><?php echo isset($coupon_data->$temp_data) ? $coupon_data->$temp_data : ""; ?></td>
                                <?php }
                                ?>
                            </tr>
                            <?php
    ...
    function get_filter_coupon($wpie_data) {
        global $wpie_product, $wpie_coupon;
        ...
        if (!empty($coupon_list)) {
            foreach ($coupon_list as $new_coupon) {
                ...
                if (isset($coupon_meta['product_ids'][0]) && $coupon_meta['product_ids'][0] != "") {
                    $coupon_product_ids = $wpie_coupon->get_coupons_product_sku($coupon_meta['product_ids'][0]);
                } else {
                    $coupon_product_ids = "";
                }
                if (isset($coupon_meta['exclude_product_ids'][0]) && $coupon_meta['exclude_product_ids'][0] != "") {
                    $exclude_product_ids = $wpie_coupon->get_coupons_product_sku($coupon_meta['exclude_product_ids'][0]);
                } else {
                    $exclude_product_ids = "";
                }
                if (isset($coupon_meta['product_categories'][0]) && $coupon_meta['product_categories'][0] != "") {
                    $coupon_product_categories = $wpie_coupon->get_product_categories_slug($coupon_meta['product_categories'][0]);
                } else {
                    $coupon_product_categories = "";
                }
                if (isset($coupon_meta['exclude_product_categories'][0]) && $coupon_meta['exclude_product_categories'][0] != "") {
                    $exclude_product_categories = $wpie_coupon->get_product_categories_slug($coupon_meta['exclude_product_categories'][0]);
                } else {
    ...
    function get_updated_coupon_fields() {
        ...
        $old_coupon_fields = $wpie_coupon->get_new_coupon_fields();
    ...
    function get_new_coupon_fields() {
        ...
        $coupon_fields = maybe_serialize($wpie_coupon->get_coupon_fields());
    ...
    function get_coupon_export_data($wpie_data = array()) {
        ...
        $coupon_field_list = $wpie_coupon->get_updated_coupon_fields();

        $coupon_list_data = $wpie_coupon->get_filter_coupon($wpie_data);
    ...
    function save_coupon_scheduled() {
        ...
        if ($wpie_export_interval != "") {
            ...
            wp_schedule_event(time(), $wpie_export_interval, 'wpie_cron_scheduled_coupon_export', array($scheduled_id));
    ...
    function wpie_save_coupon_fields() {
        ...
        $old_coupon_fields = $wpie_coupon->get_updated_coupon_fields();
        ...
        foreach ($old_coupon_fields as $coupon_fields_key => $coupon_fields_data) {
            foreach ($coupon_fields_data as $key => $value) {
                ...
                $new_fields[$coupon_fields_key][$key]['field_value'] = isset($_POST['wpie_' . $value['field_key'] . '_field']) ? $_POST['wpie_' . $value['field_key'] . '_field'] : "";
            }
        }
    ...
    function wpie_import_coupon() {
        ...
        if ($file_path != "") {
            ...
            if ($fh !== FALSE) {
                ...
                $return_value['success_log'] = '<div class="wpie_success_msg wpie_coupon_import_success_msg">' . __('Coupons Imported successfully.', WPIE_TEXTDOMAIN) . '</div>';
            } else {
            ...
            if (!empty($import_data)) {
                ...
                $coupon_updated_data = $wpie_coupon->wpie_create_new_coupon($import_data);

                $return_value['data'] = @$coupon_updated_data['data'];

                $return_value['coupon_log'] = $wpie_coupon->set_coupon_import_errors($coupon_updated_data['coupon_log']);
            }
        }
    ...
    function wpie_create_new_coupon($coupon_data = array()) {
        global $wpie_product, $wpie_coupon;
        ...
        foreach ($coupon_data as $coupon_info) {
            ...
            if (isset($coupon_info['post_title']) && $coupon_info['post_title'] != "") {
                $current_coupon_id = $wpie_coupon->get_coupon_id_from_code($coupon_info['post_title']);
            }
            ...
            if (($wpie_coupon_create_method == 'update_coupon' || $wpie_coupon_create_method == 'skip_coupon') && $current_coupon_id != "" && $current_coupon_id > 0) {
                if ($wpie_coupon_create_method == 'skip_coupon') {
                    $imported_ids['wpie_coupon_ids'][] = $current_coupon_id;

                    $wpie_coupon_log[] = sprintf(__('Coupon Code %s already Exist.', WPIE_TEXTDOMAIN), $coupon_info['post_title']);

                    continue;
                }

                if ($wpie_coupon_create_method == 'update_coupon') {
        ...
        $product_all_data['data'] = $wpie_coupon->get_imported_coupon($imported_ids);

        $product_all_data['coupon_log'] = $wpie_coupon_log;
    ...
    function get_imported_coupon($coupon_ids = array()) {
        ...
        $coupon_list = $wpie_coupon->get_filter_coupon($coupon_ids);
    ...
    function set_coupon_import_errors($coupon_errors = array()) {
        ...
        if (!empty($coupon_all_errors)) {
            foreach ($coupon_all_errors as $wpie_error) {
                ?>
                <div class="wpie_error_msg wpie_import_coupon_error_log"><?php echo $wpie_error; ?></div>
                <?php
            }
        }
