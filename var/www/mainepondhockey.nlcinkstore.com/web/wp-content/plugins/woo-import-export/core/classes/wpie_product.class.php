...
class wpie_product {
    ...
    function __construct() {
        add_action('wp_ajax_wpie_get_filter_results', array(&$this, 'wpie_get_filter_results'));
    ...
    function wpie_get_filter_results() {
        global $wpie_product;

        $wpie_product->wpie_set_time_limit(0);

        $product_list = $wpie_product->get_filter_product($_POST);
        ...
        <div class="wpie_filter_data_container">
            ...
            <table class="wpie_product_filter_data">
                <thead>
                    <tr>
                        <th></th>
    ...
    function get_filter_product($wpie_data) {
        ...
        while ($product_results->have_posts()) {
            ...
            $product_data_list[] = array(
                ...
                '_product_url' => @$product->is_type('external') ? $wpie_product->wpie_get_product_url($product) : '',
                '_button_text' => @$product->is_type('external') ? $wpie_product->wpie_get_button_text($product) : '',
            ...
            'unit' => get_option('woocommerce_dimension_unit'),
                ...
                'related_ids' => @$wpie_product->wpie_get_sku_by_id(implode(',', array_map('absint', array_values($product->get_related())))),
                '_upsell_ids' => @$wpie_product->wpie_get_sku_by_id(implode(',', array_map('absint', $product->get_upsells()))),
                '_crosssell_ids' => @$wpie_product->wpie_get_sku_by_id(implode(',', array_map('absint', $product->get_cross_sells()))),
                'parent_id' => @$product->post->post_parent,
                'product_cat' => @$wpie_product->wpie_get_categories_by_producy_id($product->id), //implode(',',wp_get_post_terms( $product->id, 'product_cat', array( 'fields' => 'names' ) )),
                'product_tag' => @implode(',', wp_get_post_terms($product->id, 'product_tag', array('fields' => 'names'))),
                'images' => @$wpie_product->wpie_get_images($product),
                ...
                '_product_attributes' => @$wpie_product->wpie_get_attributes($product),
                '_product_custom_fields' => @maybe_serialize(get_post_custom($product->id)),
                'downloads' => @$wpie_product->wpie_get_downloads($product),
                ...
                foreach ($product->get_children() as $child_id) {
                    ...
                    if ($variation->product_type == 'variation') {
                        $product_data_list[] = array(
                            ...
                            '_product_url' => @$variation->is_type('external') ? $wpie_product->wpie_get_product_url($variation) : '',
                            '_button_text' => @$variation->is_type('external') ? $wpie_product->wpie_get_button_text($variation) : '',
                        ...
                        'unit' => get_option('woocommerce_dimension_unit'),
                            ...
                            'related_ids' => @$wpie_product->wpie_get_sku_by_id(implode(',', array_map('absint', array_values($variation->get_related())))),
                            '_upsell_ids' => @$wpie_product->wpie_get_sku_by_id(implode(',', array_map('absint', $variation->get_upsells()))),
                            '_crosssell_ids' => @$wpie_product->wpie_get_sku_by_id(implode(',', array_map('absint', $variation->get_cross_sells()))),
                            'parent_id' => @$variation->post->post_parent,
                            'product_cat' => @$wpie_product->wpie_get_categories_by_producy_id($variation->id), //implode(',',wp_get_post_terms( $variation->id, 'product_cat', array( 'fields' => 'names' ) )),
                            'product_tag' => @implode(',', wp_get_post_terms($variation->id, 'product_tag', array('fields' => 'names'))),
                            'images' => @$wpie_product->wpie_get_images($variation),
                            ...
                            '_product_attributes' => @$wpie_product->wpie_get_attributes($variation),
                            '_product_custom_fields' => @maybe_serialize(get_post_custom($child_id)),
                            'downloads' => @$wpie_product->wpie_get_downloads($variation),
    ...
    function get_product_export_data($wpie_data = array()) {
        ...
        $product_field_list = $wpie_product->get_updated_product_fields();

        $product_list_data = $wpie_product->get_filter_product($wpie_data);
    ...
    function wpie_get_product_import_export_log() {
        global $wpdb;

        $results = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "wpie_export_log ORDER BY `export_log_id` DESC");

        return $results;
    }

    function wpie_remove_export_entry() {
        global $wpdb, $wpie_product;
        ...
        if ($log_id != "") {
            ...
            $return_value['message_data'] = __('Successfully Deleted.', WPIE_TEXTDOMAIN);
        }
    ...
    function wpie_import_products() {
        ...
        if ($file_path != "") {
            ...
            if (!empty($import_data)) {
                $product_updated_data = $wpie_product->wpie_create_new_product($import_data);

                $return_value['data'] = @$product_updated_data['data'];

                $return_value['product_log'] = $wpie_product->set_product_import_errors($product_updated_data['product_log']);
            }
        }
    ...
    function wpie_create_new_product($product_data = array()) {
        global $wpie_product;
        ...
        foreach ($product_data as $product_info) {
            ...
            if (isset($product_info['_sku']) && $product_info['_sku'] != "" && ($product_create_method == 'update_product' || $product_create_method == 'skip_product')) {
                ...
                if ($product_create_method == 'skip_product' && $existing_product_id != "" && $existing_product_id > 0) {
                    $imported_ids['wpie_product_ids'][] = $existing_product_id;
            ...
            if ($product_info['_product_type'] == 'simple' || $product_info['_product_type'] == 'external') {
                ...
                if ($new_product_id > 0) {
                    $wpie_product_log[] = $wpie_product->wpie_set_product_attributes($product_info, $new_product_id, $existing_product_id);
            ...
            $imported_ids['wpie_product_ids'][] = $new_product_id;
        }
        ...
        $product_all_data['data'] = $wpie_product->get_imported_product($imported_ids);
    ...
    function wpie_set_product_attributes($product_info = array(), $new_product_id = 0, $existing_product_id = "") {
        ...
        if ($product_info['_downloadable'] == 'yes' || $product_info['_downloadable'] == '1') {
            ...
            $product_log[] = $wpie_product->get_downlodable_file($new_product_id, $product_info['_downloadable_files']);
        }
...
//		}
        ...
        if ($product_info['_crosssell_ids'] != "") {
            update_post_meta($new_product_id, '_crosssell_ids', $wpie_product->get_ids_by_sku($product_info['_crosssell_ids']));
        }

        if ($product_info['_upsell_ids'] != "") {
            update_post_meta($new_product_id, '_upsell_ids', $wpie_product->get_ids_by_sku($product_info['_upsell_ids']));
        }

        if ($product_info['related_ids'] != "") {
            update_post_meta($new_product_id, 'related_ids', $wpie_product->get_ids_by_sku($product_info['related_ids']));
        }
        ...
        $product_log[] = $wpie_product->wpie_upload_remote_image($product_info['images'], $new_product_id, $has_post_thumbnail, $existing_product_id);

        return $product_log;
    }

    function get_downlodable_file($new_product_id = 0, $downloadable_files = "") {
        global $wpie_product, $download_product_errors;
    ...
    function get_imported_product($product_ids) {
        ...
        $product_list = $wpie_product->get_filter_product($product_ids);
        ...
        <div class="wpie_filter_data_container">
            <div class="wpie_product_imported_data_title_wrapper">
                <div class="wpie_product_imported_data_title"><?php _e('Created/Updated Products', WPIE_TEXTDOMAIN); ?></div>
            </div>
            <div class="wpie_product_filter_data_wrapper">
                <table class="wpie_product_filter_data">
                    <thead>
                        <tr>
                            <th></th>
    ...
    function set_product_import_errors($product_errors = array()) {
        ...
        if (!empty($product_all_errors)) {
            foreach ($product_all_errors as $wpie_error) {
                ?>
                <div class="wpie_error_msg wpie_import_error_log"><?php echo $wpie_error; ?></div>
                <?php
            }
        }
    ...
    function get_new_product_fields() {
        ...
        $product_fields = maybe_serialize($wpie_product->get_product_fields());
    ...
    function get_updated_product_fields() {
        ...
        $old_product_fields = $wpie_product->get_new_product_fields();
    ...
    function wpie_save_product_fields() {
        ...
        $old_product_fields = $wpie_product->get_updated_product_fields();
        ...
        foreach ($old_product_fields as $product_fields_key => $product_fields_data) {
            foreach ($product_fields_data as $key => $value) {
                ...
                $new_fields[$product_fields_key][$key]['field_value'] = isset($_POST['wpie_' . $value['field_key'] . '_field']) ? $_POST['wpie_' . $value['field_key'] . '_field'] : "";
            }
        }
    ...
    function save_product_scheduled() {
        ...
        if ($wpie_export_interval != "") {
            ...
            wp_schedule_event(time(), $wpie_export_interval, 'wpie_cron_scheduled_product_export', array($scheduled_id));
