...
class wpie_order {
    ...
    function __construct() {
        add_action('wp_ajax_wpie_get_order_details', array(&$this, 'wpie_get_order_details'));
    ...
    function get_order_list() {
        ...
        if (function_exists('wc_get_order_statuses')) {
            ...
            $query_args['post_status'] = array_keys($wpie_order->get_woo_order_status());
        }
    ...
    function wpie_get_order_details() {

        global $wpie_order;

        $order_field_list = $wpie_order->get_updated_order_fields();

        $order_list_data = $wpie_order->wpie_get_order_data($_POST);
        ...
        <div class="wpie_filter_data_container">
            <table class="wpie_order_filter_data">
                <thead>
                    <tr class="wpie_order_filter_data_row">
                        <th class="wpie_order_filter_data_header"></th>
                        <?php
                        foreach ($order_field_list['order_field'] as $order_field) {

                            if ($order_field['field_display'] == 1) {
                                ?>

                                <th class="wpie_order_filter_data_header"><?php echo $order_field['field_value']; ?></th>
                        ...
                        for ($i = 0; $i < $total_diff_no_product; $i++) {
                            foreach ($order_field_list['product_field'] as $order_field) {
                                ...
                                if ($order_field['field_display'] == 1) {
                                    ...
                                    <th class="wpie_order_filter_data_header"><?php echo $order_field['field_value'] . ' #' . ($i + 1); ?></th>
                ...
                <tbody>
                    ...
                    foreach ($order_list_data as $order_info) {
                        ...
                        ?>
                        <tr class="wpie_order_filter_data_row">
                            <td class="wpie_order_filter_data_column"><?php echo $temp_count_data; ?></td>
                            <?php
                            foreach ($order_field_list['order_field'] as $order_field) {

                                if ($order_field['field_display'] == 1) {
                                    $temp_data = $order_field['field_key'];
                                    ?>

                                    <td class="wpie_order_filter_data_column"><?php echo $order_info->$temp_data; ?></td>
                            ...
                            foreach ($order_info->product_list as $product_data) {
                                foreach ($order_field_list['product_field'] as $order_field) {
                                    ...
                                    if ($order_field['field_display'] == 1) {
                                        ...
                                        <td class="wpie_order_filter_data_column"><?php echo $product_data[$temp_data]; ?></td>
                            ...
                            for ($i = $temp_count; $i < $total_diff_no_product; $i++) {
                                foreach ($order_field_list['product_field'] as $order_field) {
                                    ...
                                    if ($order_field['field_display'] == 1) {
                                        ...
                                        <td class="wpie_order_filter_data_column"></td>
                            ...
                            foreach ($order_info->order_coupons_list as $coupons_data) {
                                foreach ($order_field_list['order_coupon'] as $order_field) {
                                    ...
                                    if ($order_field['field_display'] == 1) {
                                        ...
                                        <td class="wpie_order_filter_data_column"><?php echo $coupons_data[$order_field['field_key']]; ?></td>
    ...
    function wpie_get_order_data($wpie_data = array()) {
        global $wpie_order, $wpie_product;
        ...
        foreach ($order_results as $order_result) {
            ...
            $shop_order_status = $wpie_order->get_woo_order_status();
            ...
            if ($filter_flag == 1) {
                ...
                if (!empty($order_coupons_list)) {
                    foreach ($order_coupons_list as $key => $value) {
                        ...
                        $coupon_detail[$temp_count]['coupon_amount'] = $wpie_order->get_coupon_amount($order->id, $value);
    ...
    function get_order_csv_data($wpie_data = array()) {
        ...
        $order_list_data = $wpie_order->wpie_get_order_data($wpie_data);
        ...
        foreach ($order_list_data as $order_info) {
            ...
            foreach ($order_field_list['order_field'] as $field_data) {
                ...
                if ($field_data['field_display'] == 1) {
                    ...
                    $data_result[] = $order_info->$temp_data;
                }
            }
            $temp_count = 0;
            foreach ($order_info->product_list as $product_data) {
                foreach ($order_field_list['product_field'] as $field_data) {
                    ...
                    if ($field_data['field_display'] == 1) {
                        $temp_data = $field_data['field_key'];
                        $data_result[] = $product_data[$temp_data];
                    }
                }
                $temp_count++;
            }
            ...
            foreach ($order_info->order_coupons_list as $coupons_data) {
                foreach ($order_field_list['order_coupon'] as $field_data) {
                    ...
                    if ($field_data['field_display'] == 1) {
                        $temp_data = $field_data['field_key'];
                        $data_result[] = $coupons_data[$temp_data];
                    }
                }
                $temp_count++;
            }
    ...
    function wpie_import_order() {
        global $wpie_product, $wpie_order;
        ...
        if ($file_path != "") {
            ...
            if ($fh !== FALSE) {
                ...
                $return_value['success_log'] = '<div class="wpie_success_msg wpie_order_import_success_msg">' . __('Order Imported successfully.', WPIE_TEXTDOMAIN) . '</div>';
            } else {
            ...
            if (!empty($import_data)) {
                $order_updated_data = $wpie_order->wpie_create_new_order($import_data, $import_title_data);

                $error_msg = "";

                if (isset($order_updated_data['error']) && (!empty($order_updated_data['error']))) {
                    foreach ($order_updated_data['error'] as $error) {
                        $error_msg = $error_msg . '<div class="wpie_error_msg wpie_order_import_error_msg" >' . $error . '</div>';
                    }
                }
                if (isset($order_updated_data['wpie_order_ids']) && (!empty($order_updated_data['wpie_order_ids']))) {
                    $return_value['data'] = $wpie_order->wpie_get_new_order_data($order_updated_data);
                }
    ...
    function wpie_create_new_order($import_data = array(), $import_title_data = array()) {
        ...
        $order_create_option = isset($_POST['wpie_order_create_option']) ? $_POST['wpie_order_create_option'] : "skip_order";

        foreach ($import_data as $order_info) {
            $duplicate_order_id = $wpie_order->wc_get_order_id_by_order_key($order_info[36]);


            if ($duplicate_order_id > 0 && $order_create_option == 'skip_order') {
                $return_data['error'][] = sprintf(__('Order #%s already Exist.', WPIE_TEXTDOMAIN), $duplicate_order_id);

                $return_data['wpie_order_ids'][] = $duplicate_order_id;
            ...
            $return_data['wpie_order_ids'][] = $order_id;
                    ...
                    if ($order_item_id && is_array($order_item['item_meta']) && !empty($order_item['item_meta'])) {
                        if ($order_item['type'] == 'line_item') {
                            if (isset($order_info[$temp_product_count + 5]) && $order_info[$temp_product_count + 5] != "") {
                                $product_id_by_sku = $wpie_order->wc_get_product_id_by_sku($order_info[$temp_product_count + 5]);
    ...
    function wpie_get_new_order_data($wpie_order_ids) {

        global $wpie_order;

        $order_field_list = $wpie_order->get_updated_order_fields();

        $order_list_data = $wpie_order->wpie_get_order_data($wpie_order_ids);
        ...
        <div class="wpie_filter_data_container">
            <div class="wpie_product_imported_data_title_wrapper">
                <div class="wpie_product_imported_data_title"><?php _e('Created/Existing Orders', WPIE_TEXTDOMAIN); ?></div>
            </div>
                                ...
                                foreach ($order_info->product_list as $product_data) {
                                    foreach ($order_field_list['product_field'] as $order_field) {
                                        ...
                                        if ($order_field['field_display'] == 1) {
                                            ...
                                            <td class="wpie_order_filter_data_column"><?php echo $product_data[$order_field['field_key']]; ?></td>
    ...
    function get_new_order_fields() {
        ...
        $order_fields = maybe_serialize($wpie_order->order_field_list());
    ...
    function get_updated_order_fields() {
        ...
        $old_order_fields = $wpie_order->get_new_order_fields();
    ...
    function wpie_save_order_fields() {
        ...
        $old_order_fields = $wpie_order->get_updated_order_fields();
        ...
        foreach ($old_order_fields as $order_fields_key => $order_fields_data) {
            foreach ($order_fields_data as $key => $value) {
                ...
                $new_fields[$order_fields_key][$key]['field_value'] = isset($_POST['wpie_' . $value['field_key'] . '_field']) ? $_POST['wpie_' . $value['field_key'] . '_field'] : "";
            }
        }
    ...
    function save_order_scheduled() {
        ...
        if ($wpie_export_interval != "") {
            ...
            wp_schedule_event(time(), $wpie_export_interval, 'wpie_cron_scheduled_order_export', array($scheduled_id));
