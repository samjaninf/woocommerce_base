...
class WC_CP_Admin {
	...
	public function composite_layout_options( $composite_data, $post_id ) {
		...
		?><div class="options_group bundle_group bto_clearfix">
			<span class="options_group_h3">
				<?php _e( 'Layout Options', 'woocommerce-composite-products' ); ?>
			...
			<div class="bto_layouts bto_clearfix form-field">
				...
				<ul class="bto_clearfix bto_layouts_list">
					...
					foreach ( $layouts as $layout_id => $layout_description ) {
						...
						$layout_src     = apply_filters( 'woocommerce_composite_product_layout_image_src', WC_CP()->plugin_url() . '/assets/images/' . $layout_id . '.png', $post_id );
			...
			<p class="form-field">
				...
				<select name="bto_selection_mode">
					...
					$mode = get_post_meta( $post_id, '_bto_selection_mode', true );
			...
			<?php do_action( 'woocommerce_composite_admin_after_layout_options', $composite_data, $post_id ); ?>
	...
	public function composite_component_options( $composite_data, $post_id ) {
		...
		?><div class="options_group config_group bto_clearfix">
			<p class="toolbar">
				<span class="options_group_h3"><?php _e( 'Components', 'woocommerce-composite-products' ); ?>
					<img class="help_tip" data-tip="<?php echo __( 'Composite Products consist of building blocks, called <strong>Components</strong>. Each Component offers an assortment of <strong>Component Options</strong> that correspond to existing Simple products, Variable products or Product Bundles.', 'woocommerce-composite-products' ); ?>" src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" />
				</span>
				...
				<div class="bto_groups wc-metaboxes ui-sortable" data-count="">
					...
					if ( $composite_data ) {
						...
						foreach ( $composite_data as $group_id => $data ) {
							...
							/**
							 * woocommerce_composite_component_admin_html hook.
							 *
							 * @hooked component_admin_html - 10
							 */
	...
	public function scenario_actions( $id, $scenario_data, $composite_data, $product_id ) {
		...
		<div class="scenario_action_compat_group" >
			<div class="form-field">
				...
				<img class="help_tip" data-tip="<?php echo __( 'Creates a group of dependent selections from the products/variations included in this Scenario. Any selections that do not belong in this group will be disabled unless they are valid in another Scenario.', 'woocommerce-composite-products' ); ?>" src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" />
			</div>
		</div>
		<?php
						...
						<div class="bto_scenario_selector_inner" <?php echo $modifier === 'masked' ? 'style="display:none"' : ''; ?>><?php
							...
							if ( count( $component_options ) < 30 ) {
								...
								if ( $component_data[ 'optional' ] === 'yes' ) {
									...
									if ( WC_CP()->api->scenario_contains_product( $scenario_data, $component_id, -1 ) ) {
								...
								if ( WC_CP()->api->scenario_contains_product( $scenario_data, $component_id, 0 ) ) {
								...
								foreach ( $component_options as $item_id ) {
									...
									$title = WC_CP()->api->get_product_title( $item_id );
									...
									if ( $product_type === 'variable' ) {
										...
										$variation_descriptions = WC_CP()->api->get_product_variation_descriptions( $item_id );
									...
									if ( WC_CP()->api->scenario_contains_product( $scenario_data, $component_id, $item_id ) ) {
									...
									if ( $product_type === 'variable' ) {
										...
										if ( ! empty( $variation_descriptions ) ) {
											...
											foreach ( $variation_descriptions as $variation_id => $description ) {
												...
												if ( WC_CP()->api->scenario_contains_product( $scenario_data, $component_id, $variation_id ) ) {
									...
									foreach ( $scenario_data[ 'component_data' ][ $component_id ] as $product_id_in_scenario ) {
										...
										if ( $product_id_in_scenario == -1 ) {
											...
											if ( $product_in_scenario->product_type === 'variation' ) {
												$selections_in_scenario[ $product_id_in_scenario ] = WC_CP()->api->get_product_variation_title( $product_in_scenario );
											} elseif ( $product_in_scenario->product_type === 'variable' ) {
												$selections_in_scenario[ $product_id_in_scenario ] = WC_CP()->api->get_product_title( $product_in_scenario ) . ' ' . __( '&mdash; All Variations', 'woocommerce-composite-products' );
											} else {
												$selections_in_scenario[ $product_id_in_scenario ] = WC_CP()->api->get_product_title( $product_in_scenario );
											}
										}
									}
								}
			...
			<div class="form-field"><?php
				...
				if ( ! empty( $data[ 'assigned_ids' ] ) ) {
					...
					foreach ( $item_ids as $item_id ) {
						...
						$product_title = WC_CP()->api->get_product_title( $item_id );
				...
				if ( ! empty( $item_ids ) ) {
					...
					if ( count( $item_ids ) < 30 ) {
						...
						if ( $selected_default ) {
							...
							$product_title = WC_CP()->api->get_product_title( $selected_default );
						}
	...
	public function component_config_quantity_max( $id, $data, $product_id ) {
		...
		<div class="group_quantity_max">
			<div class="form-field">
				...
				<img class="help_tip" data-tip="<?php echo __( 'Set a maximum quantity for the selected Component Option. Leave the field empty to allow an unlimited maximum quantity.', 'woocommerce-composite-products' ); ?>" src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" />
			</div>
		</div>
		<?php
	}
	...
	public function composite_pricing_options() {
		...
		echo '<p class="form-field sale_price_dates_fields base_sale_price_dates_fields">
			...
			<a href="#" class="cancel_sale_schedule" style="display: inline-block; margin: 21px 0 0 10px;">'. __( 'Cancel', 'woocommerce' ) .'</a>
			<img class="help_tip" data-tip="' . __( 'The sale will end at the beginning of the set date.', 'woocommerce' ) . '" src="' . esc_url( WC()->plugin_url() ) . '/assets/images/help.png" height="16" width="16" />
		</p>';
	...
	public function composite_write_panel() {
		...
		<div id="bto_product_data" class="bto_panel panel woocommerce_options_panel wc-metaboxes-wrapper"><?php
			...
			/**
			 * woocommerce_composite_admin_html hook.
			 *
			 * @hooked composite_layout_options    - 10
			 * @hooked composite_component_options - 15
			 */
						...
						<div class="bto_scenarios wc-metaboxes"><?php
							...
							if ( $bto_scenarios ) {
								...
								foreach ( $bto_scenarios as $scenario_id => $scenario_data ) {
									...
									/**
									 * woocommerce_composite_scenario_admin_html hook.
									 *
									 * @hooked scenario_admin_html - 10
									 */
	...
	public function save_composite_config( $post_id, $posted_composite_data ) {
		...
		if ( isset( $posted_composite_data[ 'bto_data' ] ) ) {
					...
					foreach ( $ordered_bto_data as $group_id => $group_data ) {
						...
						if ( isset( $scenario_post_data[ 'modifier' ][ $group_id ] ) && $scenario_post_data[ 'modifier' ][ $group_id ] === 'not-in' ) {
							...
							if ( ! empty( $scenario_post_data[ 'component_data' ][ $group_id ] ) ) {
								...
								if ( WC_CP()->api->scenario_contains_product( $scenario_post_data, $group_id, 0 ) ) {
						...
						if ( ! empty( $scenario_post_data[ 'component_data' ][ $group_id ] ) ) {
							...
							if ( WC_CP()->api->scenario_contains_product( $scenario_post_data, $group_id, -1 ) ) {
			...
			if ( ! empty( $ordered_bto_scenario_data ) ) {
				...
				$scenarios_for_products = WC_CP()->api->build_scenarios( $ordered_bto_scenario_data, $bto_data );
		...
		if ( $posts ) {
			foreach ( $posts as $post ) {
				...
				if ( $product->product_type === 'variation' ) {
					$found_products[ $post ] = WC_CP()->api->get_product_variation_title( $product );
				} else {
					if ( $x === 'search_component_options_in_scenario' && $product->product_type === 'variable' ) {
						$found_products[ $post ] = WC_CP()->api->get_product_title( $product ) . ' ' . __( '&mdash; All Variations', 'woocommerce-composite-products' );
					} else {
						$found_products[ $post ] = WC_CP()->api->get_product_title( $product );
					}
				}
