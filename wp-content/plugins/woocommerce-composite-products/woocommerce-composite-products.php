...
class WC_Composite_Products {
	...
	public function plugins_loaded() {
		...
		// PB check.
		if ( ! empty( $woocommerce_bundles ) && version_compare( $woocommerce_bundles->version, $this->required_pb ) < 0 ) {
	...
	/**
	 * Displays a warning message if PB version check fails.
	 *
	 * @return string
	 */
	public function pb_admin_notice() {

		echo '<div class="error"><p>' . sprintf( __( 'Please update WooCommerce Product Bundles to version %1$s or higher. The version found on your system does not support WooCommerce Composite Products %2$s.', 'woocommerce-composite-products' ), $this->required_pb, $this->version ) . '</p></div>';
	}

	/**
	 * Displays a warning message if WC version check fails.
	 *
	 * @return string
	 */
	public function wc_admin_notice() {

		echo '<div class="error"><p>' . sprintf( __( 'WooCommerce Composite Products requires at least WooCommerce %s in order to function. Please upgrade WooCommerce.', 'woocommerce-composite-products' ), $this->required_pb ) . '</p></div>';
	}
